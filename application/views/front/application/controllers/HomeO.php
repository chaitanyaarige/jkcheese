...
class Home extends CI_Controller {
	...
	public function show_products_by_category()
	{
		...
		$_SESSION['maincat_id'] = $data['maincat_id'];
		$data['maincat_name'] =$this->HomeModel->get_maincat_name($data['maincat_id']);
		$_SESSION['maincat_name'] = $data['maincat_name'];
	...
	{	
		...
		$data['feature'] = $this->load->view('front/product_list',$data,true);
		$data['maincat_id']= $_SESSION['maincat_id'];
		$data['maincat_name'] = $_SESSION['maincat_name'];
		$this->load->view('front/product_by_cat',$data);
	}
	...
	function phpmailer()
{
	...
	public function reset_password()
	{
		...
		if($data==true)
		{
			...
			<a href="'.base_url("/confirm-account/$id").'" style="color:red;font-size:20px;">Activate My Account</a>';
			$this->load->library("PhpMailerLib");
        $mail = $this->phpmailerlib->load();
	try {
		    //Server settings
		    ...
		    $mail->Port = 465;                                    // TCP port to connect to
		    //Recipients
		    $mail->setFrom('faiz@alakopdigital.com', 'JKC');
		    $mail->addAddress('faiz.khan9121993@gmail.com', 'faiz');     // Add a recipient
		    $mail->addAddress($email);               // Name is optional
		    $mail->addReplyTo($email, 'Ganesha');
		    //$mail->addCC('cc@example.com');
