...
class HomeModel extends CI_Model {
	...
	public function horeca_post_by_cat_name($name)
	{
		$data = $this->db->select('*')
			->from('tbl_horeca')
			->where('cat',$name)
	...
	public function horeca_post_by_sub_cat_name($cat,$subcat)
	{
		$data = $this->db->select('*')
			->from('tbl_horeca')
			->where('cat',$cat)
			->where('sub_cat',$subcat)
			->get()
			->result();
			return $data;
	}

	public function subcat_by_brand_id($brand_id){
		...
		$data = $this->db->select('*')
			...
			->order_by("pro_id", "asc")
			->limit(1)->get()->row();
		return  ($data) ? $data->pro_sub_cat : NULL;
	}
